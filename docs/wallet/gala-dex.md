...
2. [Tokens & Pool Constants](#tokens--pool-constants)
3. [Wallet Address Derivation](#wallet-address-derivation)
4. [Signing Payloads](#signing-payloads)
5. [Deterministic JSON Serialization](#deterministic-json-serialization)
6. [DEX API Endpoints](#dex-api-endpoints)
   - [Get Pool State](#get-pool-state)
   - [Get Position Fees](#get-position-fees)
   - [Get Transaction Status](#get-transaction-status)
   - [Generate Swap Payload](#generate-swap-payload)
7. [Bundle API — Submitting Transactions](#bundle-api--submitting-transactions)
8. [Operations](#operations)
   - [Add Liquidity](#add-liquidity)
   - [Remove Liquidity](#remove-liquidity)
   - [Swap (exact-input)](#swap-exact-input)
   - [Collect Position Fees](#collect-position-fees)
9. [Transaction Confirmation](#transaction-confirmation)
10. [Uniswap v3 Liquidity Math](#uniswap-v3-liquidity-math)
11. [GALA Price from Pool](#gala-price-from-pool)
...
See [Transaction Confirmation](#transaction-confirmation) for status values and polling logic.
...
**Step 1 — compute amounts from current sqrtPrice** (see [Math section](#uniswap-v3-liquidity-math)):
```
...
```json
{
  ...
  "amount0Min":     "<gala_amount × 0.995>",
  "amount1Min":     "<gusdc_amount × 0.995>",
  "positionId":     "<your_position_id>",
  "uniqueKey":      "galaswap - operation - <uuid-v4>"
}
```

**Step 3 — submit to Bundle API:**
```json
{
  "method":    "AddLiquidity",
  "signedDto": { "<above payload + signature>" },
  "stringsInstructions": [
    "$pool$GUSDC$Unit$none$none$GALA$Unit$none$none$10000",
    "$userPosition$eth|<wallet>",
    "$tokenBalance$GALA$Unit$none$none$eth|<wallet>",
    "$tokenBalance$GUSDC$Unit$none$none$eth|<wallet>",
    "$tokenBalance$GALA$Unit$none$none$$pool$GUSDC$Unit$none$none$GALA$Unit$none$none$10000",
    "$tokenBalance$GUSDC$Unit$none$none$$pool$GUSDC$Unit$none$none$GALA$Unit$none$none$10000"
  ]
}
```
...
Returns a `txID` in `PENDING` state.

---

### Swap (exact-input)

Swap an exact amount of one token for the other. This is a two-step process.

**Step 1 — compute `sqrtPriceLimit`** from pool's current `sqrtPrice`:

```
Selling GALA  (tokenIn=GALA):  sqrtPriceLimit = sqrtPrice × (1 - 0.05)
Selling GUSDC (tokenIn=GUSDC): sqrtPriceLimit = sqrtPrice × (1 + 0.05)
```

**Step 2 — request unsigned DTO from DEX API:**

```
POST /v1/trade/swap

{
  "tokenIn":          { "collection": "GALA",  ... },
  "tokenOut":         { "collection": "GUSDC", ... },
  "fee":              10000,
  "amountIn":         "<amount>",
  "amountInMaximum":  "<amount>",
  "amountOutMinimum": "-0.000001",
  "sqrtPriceLimit":   "<computed above>"
}
```

Response: `{ "data": { <unsigned DTO> } }`

**Step 3 — sign the DTO and submit to Bundle API:**

```json
{
  "method":    "Swap",
  "signedDto": { "<DTO from step 2 + signature>" },
  "stringsInstructions": [
    "$pool$GUSDC$Unit$none$none$GALA$Unit$none$none$10000",
    "$tokenBalance$<tokenIn_key>$eth|<wallet>",
    "$tokenBalance$<tokenOut_key>$eth|<wallet>",
    "$tokenBalance$<tokenIn_key>$$pool$GUSDC$Unit$none$none$GALA$Unit$none$none$10000",
    "$tokenBalance$<tokenOut_key>$$pool$GUSDC$Unit$none$none$GALA$Unit$none$none$10000"
  ]
}
```
...
Claims accrued trading fees from your pool position.

**Step 1 — fetch pending fees** via `GET /v1/trade/position` (see above). Read `tokensOwed0` (GALA) and `tokensOwed1` (GUSDC).
...
while now < deadline:
    sleep(5s)
    response = GET /v1/trade/transaction-status?id={txID}
    status   = response.data.status.toUpperCase()
    if status in ["SUCCESS", "PROCESSED"]:
        return OK
    if status in ["FAILED", "ERROR"]:
        reason = response.data.batch.error
        return ERROR(reason)
return TIMEOUT
```
...
amount0  = L × (1/sqrtPrice - 1/sqrtPriceUpper)   → GALA required
amount1  = gusdcAmount                              → GUSDC you deposit
liquidity = L
```

Both `amount0` and `amount1` should be formatted to 18 decimal places.

### Remove Liquidity — given liquidity units

```
amount0 = L × (1/sqrtPrice - 1/sqrtPriceUpper)   → GALA redeemable
amount1 = L × (sqrtPrice   - sqrtPriceLower)      → GUSDC redeemable
```

These are estimates — actual amounts depend on the on-chain state at confirmation time.

### Notes

- Use arbitrary-precision decimal arithmetic (not float64) to avoid rounding errors.
- Liquidity `L` is a very large number — handle it with a big-decimal library.
- The `amount` field in `RemoveLiquidity` must be the **floored integer** of `L` (no decimal part).
...
```
price_GUSDC_per_GALA = sqrtPrice²
```
...
```
sqrtPrice = "1.234"
price     = 1.234 × 1.234 = 1.522756 GUSDC per GALA
```
